...
export default function AdminDashboardClient() {
  ...
  const checkAuthAndFetch = useCallback(async () => {
    try {
      ...
      if (!roleData) {
        toast.error("Admin access required. Please contact administrator.");
        await supabase.auth.signOut();
        router.push("/admin");
        return;
      }
      ...
      console.error('Unexpected error in checkAuthAndFetch:', error);
  ...
  const fetchData = async () => {
    try {
      ...
      if (applicationsRes.error) {
        console.error('Applications fetch error:', applicationsRes.error);
        throw new Error(`Failed to load applications: ${applicationsRes.error.message}`);
      }
      if (quotesRes.error) {
        console.error('Quotes fetch error:', quotesRes.error);
        throw new Error(`Failed to load quotes: ${quotesRes.error.message}`);
      }
      ...
      <main className="lg:ml-72 min-h-screen relative overflow-hidden">
        ...
        <div className="p-4 sm:p-6 lg:p-8 relative z-10 max-w-7xl mx-auto">
          ...
          <motion.div
            ...
            className="hidden lg:flex items-center justify-between mb-8 p-4 rounded-2xl bg-card/60 backdrop-blur-xl border border-border/50 shadow-lg"
          >
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="w-2.5 h-2.5 rounded-full bg-primary animate-pulse" />
                <h1 className="font-display text-2xl sm:text-3xl font-extrabold tracking-tight">{meta.title}</h1>
                <Badge variant="outline" className="ml-2 border-primary/30 text-primary text-xs bg-primary/10">
                  Live Admin
                </Badge>
              </div>
              <p className="text-muted-foreground text-xs sm:text-sm">{meta.desc}</p>
            </div>
            
            <div className="flex items-center gap-3">
              <div className="hidden xl:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-background/80 border border-border/50 text-xs text-muted-foreground">
                <Shield size={14} className="text-primary" />
                <span>Encrypted Connection</span>
              </div>
              <Button variant="outline" onClick={handleRefresh} disabled={refreshing} className="gap-2 rounded-xl border-primary/20 hover:border-primary/40">
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === "drip-campaigns" && <DripCampaignTab />}
