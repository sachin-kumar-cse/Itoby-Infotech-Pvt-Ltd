...
const SUPABASE_URL = "https://uvpxfbucgcpsjwahmvjy.supabase.co";
...
`
  },
  {
    ...
    category: "FinTech & Invoicing",
    image: "/images/blog/retail-digital-whatsapp-invoicing.webp",
    author: "FinTech Team",
...
`
  },
  {
    ...
    title: "Why Itoby Infobyte Pvt Ltd (IIPL) is Scaling Global SaaS & Custom Software Engineering",
...
async function seed() {
  console.log("Seeding 20 SEO-rich blogs into Supabase database...");
  for (const b of blogs) {
    const { data, error } = await supabase.from("blog_posts").upsert(b, { onConflict: "slug" });
    if (error) {
      console.error(`Error inserting ${b.slug}:`, error.message);
    } else {
      console.log(`Successfully upserted: ${b.title}`);
    }
  }
